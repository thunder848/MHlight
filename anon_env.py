...
class AnonEnv:
    ...
    def __init__(self, path_to_log: object, path_to_work_directory: object, dic_traffic_env_conf: object) -> object:
        self.path_to_log = path_to_log  # path_to_log是.../round_i/generator_i/
        self.path_to_work_directory = path_to_work_directory  # record/0515.../anon.../
        ...
        # touch new inter_{}.pkl (if exists, remove)，records下的 inter_X文件写入
        for inter_ind in range(self.dic_traffic_env_conf["NUM_INTERSECTIONS"]): # 写入inter_X.pkl文件
    ...
    def reset(self):
        ...
        with open(os.path.join(self.path_to_work_directory,"cityflow.config"), "w") as json_file:
            json.dump(cityflow_config, json_file) # 将 cityflow_config 对象化为 JSON 格式，并写入到打开的文件 json_file 中。
        ...
        # initialize intersections (grid)  records下的 single_inter_intersection文件写入
    ...
    def batch_log(self, start, stop):
        for inter_ind in range(start, stop):
            ...
            path_to_log_file = os.path.join(self.path_to_log, "inter_{0}.pkl".format(inter_ind))
            f = open(path_to_log_file, "wb")
            pickle.dump(self.list_inter_log[inter_ind], f)  # 将临时数据写入到inter_{0}.pkl文件中。
            f.close()
